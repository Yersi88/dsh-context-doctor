...
function colorOf(tokens: number): string {
  if (tokens < 10_000) return '#22c55e' // 绿：健康
  if (tokens < 30_000) return '#eab308' // 黄：偏高
  return '#ef4444' // 红：膨胀
}
    ...
    >
      ...
      >
        <svg width={26} height={26} viewBox="0 0 24 24" role="img" aria-hidden="true">
          <circle cx={12} cy={12} r={radius} fill="none" stroke="rgba(128,128,128,0.3)" strokeWidth={2.5} />
          <circle
            cx={12}
            cy={12}
            r={radius}
        ...
        <span style={{ fontSize: 11, color: state.state === 'error' ? '#ef4444' : '#666', whiteSpace: 'nowrap' }}>
          {state.state === 'loading'
            ? t('cd.loading')
            : state.state === 'error'
              ? '!'
      ...
      {open && (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <strong>{t('cd.title')}</strong>
            <span style={{ fontSize: 11, color: '#888' }}>{t('cd.residentTokens')}</span>
          </div>

          {state.state === 'error' && (
            <div style={{ color: '#ef4444', marginBottom: 6 }}>{t('cd.error')}: {state.error}</div>
          )}

          {report === null && state.state !== 'loading' && (
            <div style={{ color: '#888' }}>{t('cd.empty')}</div>
          )}

          {report !== null && (
            <>
              <Row label={t('cd.instructions')} value={`${formatK(report.injected.instructions.totalTokens)}t`} />
              <Row
                label={t('cd.skills')}
                value={`${formatK(report.injected.skills.catalogDescriptionTokens)}t / ${t('cd.catalog', { n: report.injected.skills.catalogCount })}`}
              />
              <Row label={t('cd.tools')} value={`${formatK(report.injected.tools.schemaTokens)}t`} />
              <Row
                label={t('cd.mcp')}
                value={report.injected.tools.mcp.totalTools > 0
                  ...
                  : '—'}
              />
              <div style={{ marginTop: 8, color: report.suggestions.length > 0 ? '#b45309' : '#16a34a' }}>
                {report.suggestions.length > 0
                  ? t('cd.suggestions', { n: report.suggestions.length })
                  : '✓ healthy'}
              </div>
              <div style={{ marginTop: 4, maxHeight: 120, overflowY: 'auto', color: '#666', lineHeight: 1.5 }}>
                {report.suggestions.slice(0, 4).map((s, i) => (
                  <div key={i}>· [{s.severity}] {s.text}</div>
                ))}
              </div>
            </>
          )}
